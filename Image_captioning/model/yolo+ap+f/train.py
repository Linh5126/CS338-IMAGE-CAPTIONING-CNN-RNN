...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train(train_loader, encoder, decoder, criterion,
              encoder_optimizer, decoder_optimizer, epoch, scaler)
...
def train(train_loader, encoder, decoder, criterion,
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # AMP autocast — tự động dùng fp16 trên CUDA, fp32 trên CPU
        with autocast(enabled=use_amp):
            ...
            # Attention regularization với time mask
            max_T     = alphas.size(1)
        ...
        # Unscale trước khi clip để grad_clip hoạt động đúng
        ...
        if grad_clip is not None:
            nn.utils.clip_grad_norm_(
                filter(lambda p: p.grad is not None, decoder.parameters()),
                grad_clip
            )
            if encoder_optimizer is not None:
                nn.utils.clip_grad_norm_(
                    filter(lambda p: p.grad is not None, encoder.parameters()),
                    grad_clip
                )
        ...
        if i % print_freq == 0:
            print(
                ...
                f'Batch {batch_time.val:.3f}s | '
                f'Loss {losses.val:.4f} ({losses.avg:.4f}) | '
                f'Top-5 {top5accs.val:.3f} ({top5accs.avg:.3f})'
            )
