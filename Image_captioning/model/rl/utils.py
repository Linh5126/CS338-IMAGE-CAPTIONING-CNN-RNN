...
def save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder,
                    ...
                    meteor=None, rouge_l=None,
                    output_dir='/kaggle/working/'):
    """Saves model checkpoint — including METEOR and ROUGE-L alongside BLEU-4."""
    ...
    if is_best:
        torch.save(
            state,
            os.path.join(output_dir, 'BEST_checkpoint_' + data_name + '.pth.tar')
        )
        print(f"  ✓ Saved BEST checkpoint  BLEU-4={bleu4:.4f}"
              + (f"  METEOR={meteor:.4f}" if meteor is not None else "")
              + (f"  ROUGE-L={rouge_l:.4f}" if rouge_l is not None else ""))
